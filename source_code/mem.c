...
void * best_fit_allocator(unsigned int size) {
	...
	int found =0;
	struct mem_region * current_region = NULL;
	struct mem_region * p_region=free_regions;
	while (p_region != NULL)
	{
		if (size <= p_region->size) {
			found = 1;
			if( current_region==NULL || p_region->size<current_region->size)
			{
				current_region=p_region;
			}
		}
		p_region=p_region->next;
	}
